...
const {
    models: { users, question_answered_model },
} = sequelize;
module.exports = (app, passport) => {
    ...
    app.post("/protected/getUser", authPass, async (req, res) => {
        ...
        if (req.user.role === "m" || req.user.role === "su") {
            try {
                ...
                await users
                    .findAll({ where: { uuid: req.body.uuid } })
                    .then(async (d1) => {
                        await Promise.all(
                            d1.map(async (e) => {
    ...
    app.get("/protected/getUsers", authPass, async (req, res) => {
        if (req.user.role === "m" || req.user.role === "su") {
            try {
                ...
                return res
                    .status(200)
                    .json({ data: userArr, user: req.user.username });
            } catch (e) {
                res.sendStatus(404);
                console.log(e);
            }
    ...
    app.put("/protected/update", authPass, async (req, res) => {
        var a = req.body;
    ...
    app.put("/protected/feedback", authPass, async (req, res) => {
        if (req.user.role === "su" || req.user.role === "m") {
            const entry = await users.findOne({
                where: { uuid: req.body.uuid },
            });
